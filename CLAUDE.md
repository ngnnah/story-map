...
`null` for no wall. It is set by one control — a number, a Finished-this-chapter
button and a step-back — at the front of the bottom bar.

It was briefly a second draggable handle on the rail, and that failed: asking
anyone to hold two positions at once (how far you have read, versus where you
are looking) was the whole problem, and no amount of relabelling fixed it. If
you are tempted to reintroduce a second handle, don't. A book with `"readAlong": true` defaults to 0 — nothing
